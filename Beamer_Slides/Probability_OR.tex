...
\foreach \x in {0,1,...,9}
\draw [fill=black] (\x,0) circle [radius = 4pt] node [below, yshift=-0.1cm] {$\x$};
\onslide<2->{
	\foreach \x in {1,2,...,9}
	\draw[color=red,fill=red] (\x,0) circle [radius = 4pt];
}
\onslide<4->{\draw[color=yellow,fill=yellow] (0,0) circle [radius = 4pt];}
\end{tikzpicture}
\end{center}
\onslide<2->{\emph{At least one} means 1 or 2 or 3 or 4 or ...} \newline\\
\onslide<3->{Rather than add all of the probabilities of those events up, we can instead find the \alert{complement} of \emph{at least one}}.	\newline\\
\onslide<4->{The complement of \emph{at least one} is {\color{blue}\textbf{none}}.}	\newline\\
\onslide<5->{In general, the complement of \emph{at least n events} is \alert{$n-1$ events or less.}}
\end{frame}
